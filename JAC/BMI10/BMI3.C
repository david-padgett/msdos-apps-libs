...
int BMI_Input(BMI_INPUT_DESCRIPTOR * Input, CARDINAL_16 * Key) {
   ...
   if (Input == NULL) {
      ...
      if (Input->Refresh_Flag) {
         ...
         if (Input->Type == STRING_TYPE || (Input->Type == INTEGER_16_TYPE && atoi(Input->Value) != 0)) {

            sprintf(str, "%-*.*s", Input->Length, Input->Length, Input->Value);

         }
         else {

            sprintf(str, "%-*.*s", Input->Length, Input->Length, "");
         ...
         switch (*(Key)) {
            ...
            case KB_BACKSPACE :
               ...
               if (Input->Current_Offset > 0) {
                  ...
                  memmove(&(Input->Value[Input->Current_Offset]), &(Input->Value[Input->Current_Offset + 1]), Input->Length - Input->Current_Offset);
	       ...
	       break;
            ...
            default :
               ...
               if (Input->Type == STRING_TYPE) {
                  ...
                  if ((char) *(Key) >= ' ' && (char) *(Key) <= '~' && Input->Current_Offset < Input->Length) {

                     Input->Value[Input->Current_Offset] = (char) *(Key);
                     TEXT_Set_Cursor_Position(Input->Current_Row, Input->Current_Column + Input->Current_Offset, Input->Text);
                     Input->Current_Offset += (Input->Current_Offset < Input->Length - 1) ? 1 : 0;
                  ...
                  if ((((char) *(Key) >= '0' && (char) *(Key) <= '9') || (char) *(Key) == '+' || (char) *(Key) == '-') && Input->Current_Offset < Input->Length) {
               ...
               if (Input->Current_Offset + 1 == Input->Length) {
...
int BMI_Load_Edit_Parameters(char * File_Name, BMI_EDIT_EDIT_PARAMETERS * Process_Parameters) {
   ...
   FILE *  load_file;
   BOOLEAN exit_flag;
   char    whole_file_name[MAXPATH], str[256], value[6];
   int     i, result;
      ...
      if ((load_file = fopen(whole_file_name, "rt")) == NULL) {
         ...
         for(exit_flag = FALSE; !exit_flag && fgets(str, sizeof(str) - 1, load_file) != NULL;) {
            ...
            if (strncmp(str, "PN", 2) == 0) {
               ...
               strncpy(Process_Parameters->Part_Number.Value, str + 3, Process_Parameters->Part_Number.Length);
               ...
               if (strncmp(str, "CD", 2) == 0) {
                  ...
                  if (atoi(value) < Process_Parameters->Valve_Pack_Number.Minimum_Value || atoi(value) > Process_Parameters->Valve_Pack_Number.Maximum_Value) {

                     sprintf(value, "%*u", Process_Parameters->Valve_Pack_Number.Length, Process_Parameters->Valve_Pack_Number.Minimum_Value);

                  }

                  sprintf(Process_Parameters->Valve_Pack_Number.Value, "%*u", Process_Parameters->Valve_Pack_Number.Length, atoi(value));
                  ...
                  if (strncmp(str, "MP", 2) == 0) {
                     ...
                     if (atoi(value) < Process_Parameters->Multiple_Part_Number.Minimum_Value || atoi(value) > Process_Parameters->Multiple_Part_Number.Maximum_Value) {

                        sprintf(value, "%*u", Process_Parameters->Multiple_Part_Number.Length,  Process_Parameters->Multiple_Part_Number.Minimum_Value);

                     }

                     sprintf(Process_Parameters->Multiple_Part_Number.Value, "%*u", Process_Parameters->Multiple_Part_Number.Length, atoi(value));
                     ...
                     if (strncmp(str, "LB", 2) == 0) {
                        ...
                        if (atoi(value) < Process_Parameters->Last_Bin_Sensor.Minimum_Value || atoi(value) > Process_Parameters->Last_Bin_Sensor.Maximum_Value) {

                           sprintf(value, "%*u", Process_Parameters->Last_Bin_Sensor.Length, Process_Parameters->Last_Bin_Sensor.Minimum_Value);

                        }

                        sprintf(Process_Parameters->Last_Bin_Sensor.Value, "%*u", Process_Parameters->Last_Bin_Sensor.Length, atoi(value));
                        ...
                        if (strncmp(str, "TC", 2) == 0) {
                           ...
                           if (atoi(value) < Process_Parameters->Total_Cycle_Time.Minimum_Value || atoi(value) > Process_Parameters->Total_Cycle_Time.Maximum_Value) {

                              sprintf(value, "%*u", Process_Parameters->Total_Cycle_Time.Length, Process_Parameters->Total_Cycle_Time.Minimum_Value);

                           }

                           sprintf(Process_Parameters->Total_Cycle_Time.Value, "%*u", Process_Parameters->Total_Cycle_Time.Length, atoi(value));
                           ...
                           if (strncmp(str, "VA", 2) == 0) {
                              ...
                              if (atoi(value) < Process_Parameters->Advance_Time[i].Minimum_Value || atoi(value) > Process_Parameters->Advance_Time[i].Maximum_Value) {

                                 sprintf(value, "%*u", Process_Parameters->Advance_Time[i].Length, Process_Parameters->Advance_Time[i].Minimum_Value);

                              }

                              sprintf(Process_Parameters->Advance_Time[i].Value, "%*u", Process_Parameters->Advance_Time[i].Length, atoi(value));
                              Process_Parameters->Advance_Time[i].Integer_Value = atoi(value);

                              if (Process_Parameters->Retract_Time[i].Integer_Value <= atoi(value)) {

                                 sprintf(Process_Parameters->Retract_Time[i].Value, "%*u", Process_Parameters->Retract_Time[i].Length, atoi(value) + 1);
                              ...
                              if (strncmp(str, "VR", 2) == 0) {
                                 ...
                                 if (atoi(value) < Process_Parameters->Retract_Time[i].Minimum_Value || atoi(value) > Process_Parameters->Retract_Time[i].Maximum_Value) {

                                    sprintf(value, "%*u", Process_Parameters->Retract_Time[i].Length, Process_Parameters->Advance_Time[i].Integer_Value);

                                 }

                                 sprintf(Process_Parameters->Retract_Time[i].Value, "%*u", Process_Parameters->Retract_Time[i].Length, atoi(value));
...
int BMI_Save_Edit_Parameters(char * File_Name, BMI_EDIT_EDIT_PARAMETERS * Process_Parameters) {
   ...
   if (File_Name != NULL && File_Name[0] != '\0') {
      ...
      if ((save_file = fopen(whole_file_name, "wt")) == NULL) {
         ...
         fprintf(save_file, "PN,%-*.*s\n", Process_Parameters->Part_Number.Length, Process_Parameters->Part_Number.Length, Process_Parameters->Part_Number.Value);
         fprintf(save_file, "CD,%0*u\n", Process_Parameters->Valve_Pack_Number.Length, Process_Parameters->Valve_Pack_Number.Integer_Value);
         fprintf(save_file, "MP,%0*u\n", Process_Parameters->Multiple_Part_Number.Length, Process_Parameters->Multiple_Part_Number.Integer_Value);
         fprintf(save_file, "LB,%0*u\n", Process_Parameters->Last_Bin_Sensor.Length, Process_Parameters->Last_Bin_Sensor.Integer_Value);
         fprintf(save_file, "TC,%0*u\n", Process_Parameters->Total_Cycle_Time.Length, Process_Parameters->Total_Cycle_Time.Integer_Value);

         for (i = 0; i < 80; i++) {

            fprintf(save_file, "VA,%2u,%0*u\n", i + 1, Process_Parameters->Advance_Time[i].Length, Process_Parameters->Advance_Time[i].Integer_Value);
            fprintf(save_file, "VR,%2u,%0*u\n", i + 1, Process_Parameters->Retract_Time[i].Length, Process_Parameters->Retract_Time[i].Integer_Value);

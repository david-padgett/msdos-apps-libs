...
/*
   ...
   Procedure   : AAI_Activate_Process
   Description : Activates the specified process.
   Inputs      : Parameters_List = The parameters list to use while the AAI system is active.
...
/*
   ...
   Function    : AAI_Validate_Parameters
...
/*
   ...
   Function    : AAI_Allocate_Parameters_List
...
int AAI_Display_Time(void ** Process_Parameters, CARDINAL_16 * Key) {
   ...
   AAI_DISPLAY_TIME_PARAMETERS * parameters;
   time_t                        timer;
...
/*
   ...
   Function    : AAI_Load_Configuration
   ...
   if (Config_File_Name == NULL || Default_Parameters == NULL) {
      ...
      */
      ...
      if ((config_fp = fopen(Config_File_Name, "rb")) == NULL) {
         ...
         if (fread(Default_Parameters, sizeof(*(Default_Parameters)), 1, config_fp) != 1 || strcmp(Default_Parameters->ID, AAI_CONFIG_FILE_TITLE) != 0) {
...
/*
   ...
   Function    : AAI_Save_Configuration
   ...
   if (Config_File_Name == NULL || Default_Parameters == NULL) {
      ...
      */
      ...
      if ((config_fp = fopen(Config_File_Name, "wb")) == NULL) {
         ...
         strcpy(Default_Parameters->ID, AAI_CONFIG_FILE_TITLE);
...
int AAI_Yes_No_Menu(void ** Process_Parameters, CARDINAL_16 * Key) {
   ...
   AAI_YES_NO_MENU_PARAMETERS * parameters;
               ...
               *(Key) = 0;
               ...
               if (!process_aborted_flag && !option_selected_flag) {
                  ...
                  if ((result = MENU_Destructor(&(parameters->Menu))) == MENU_NOERR) {
                     ...
                     AAI_Activate_Process(parameters->Parameters_List, parameters->Invoking_Process);
                     AAI_Halt_Process(parameters->Parameters_List, AAI_YES_NO_MENU_PROCESS);
...
/*
   ...
   Procedure  : AAI_Clear_Edit_Parameters
   Description : Allows information to be entered via the keyboard.
   Inputs      : Nothing.
...
/*
   ...
   Function    : AAI_Load_Edit_Parameters
...
/*
   ...
   Function    : AAI_Save_Edit_Parameters
   Description : Saves the edit parameters to disk.
	Inputs      : File_Name          =
                 Process_Parameters =
   Updates     : Nothing.
...
void AAI_Set_Help_Screen(AAI_PARAMETERS_LIST * Process_Parameters, char * Key) {
   ...
   AAI_HELP_TOPIC_PARAMETERS * help_topic_parameters;
	...
	help_topic_parameters = Process_Parameters->Process_Descriptor[AAI_HELP_TOPIC_PROCESS].Process_Parameters;

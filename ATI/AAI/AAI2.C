...
int aaiMeasureBatch(void ** processParameters, CARDINAL_16 * key) {
   ...
   BOOLEAN                     newProcess, newFieldFlag, exitFlag;
   CARDINAL_16                 inputField, previousField;
   INTEGER_16                  direction;
   int                         result;
         ...
         */
         ...
         if (parameters->port != NULL) {
            ...
            if ((result = PORT_Constructor(parameters->startingRow, parameters->startingColumn, parameters->height, parameters->width, PORT_SINGLE_LINE_BORDER, parameters->colors.primaryColor, parameters->colors.primaryColor, &(parameters->port))) == PORT_NOERR) {
               ...
               TEXT_Set_Cursor_Position(1, 33, parameters->text);
               TEXT_Print_String("Batch Data", parameters->text);

               TEXT_Set_Cursor_Position(3, 4, parameters->text);
               TEXT_Print_String("Laboratory", parameters->text);
               parameters->laboratory.refreshFlag = TRUE;

               TEXT_Set_Cursor_Position(5, 4, parameters->text);
               TEXT_Print_String("Operator", parameters->text);
               parameters->operator.refreshFlag = TRUE;

               TEXT_Set_Cursor_Position(7, 4, parameters->text);
               TEXT_Print_String("Date & Time", parameters->text);
               parameters->dateAndTime.refreshFlag = TRUE;

               TEXT_Set_Cursor_Position(9, 4, parameters->text);
               TEXT_Print_String("Program", parameters->text);
               parameters->program.refreshFlag = TRUE;

               TEXT_Set_Cursor_Position(11, 4, parameters->text);
               TEXT_Print_String("Type of Sample", parameters->text);
               parameters->typeOfSample.refreshFlag = TRUE;

               TEXT_Set_Cursor_Position(13, 4, parameters->text);
               TEXT_Print_String("Batch Code", parameters->text);
               parameters->batchCode.refreshFlag = TRUE;

               aaiInput(&(parameters->laboratory), key);
               aaiInput(&(parameters->operator), key);
               aaiInput(&(parameters->dateAndTime), key);
               aaiInput(&(parameters->program), key);
               aaiInput(&(parameters->typeOfSample), key);
               aaiInput(&(parameters->batchCode), key);
         ...
         if (result == EZERO) {
            ...
            previousField = parameters->field;
            exitFlag = FALSE;
            direction = 1;
            ...
            switch (*(key)) {
               ...
               case KB_CTRL_GRAY_END :
                  ...
                  parameters->field = AAI_TOTAL_MEASURE_BATCH_FIELDS;
                  break;

               case KB_ESC :

                  newProcess = AaiMainMenuProcess;
                  exitFlag = TRUE;
                  break;

            }

            newFieldFlag = (previousField != parameters->field);
            inputField = parameters->field;

            switch (inputField) {

               case 2 :

                  parameters->currentFieldDescriptor = &(parameters->operator);
                  break;

               case 3 :

                  parameters->currentFieldDescriptor = &(parameters->dateAndTime);
                  break;

               case 4 :

                  parameters->currentFieldDescriptor = &(parameters->program);
                  break;

               case 5 :

                  parameters->currentFieldDescriptor = &(parameters->typeOfSample);
                  break;

               case 6 :

                  parameters->currentFieldDescriptor = &(parameters->batchCode);
                  break;

               case 1 :
               default :

                  parameters->currentFieldDescriptor = &(parameters->laboratory);
                  break;

            }

            aaiInput(parameters->currentFieldDescriptor, key);

            if (exitFlag) {

               PORT_Destructor(&(parameters->port));
               aaiHaltProcess(parameters->parametersList, AaiMeasureBatchProcess);
               aaiActivateProcess(parameters->parametersList, newProcess);

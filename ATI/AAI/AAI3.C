/*
   ...
   System      : AAI
   Description : Miscellaneous support procedures and functions for the AAI
                 system.
...
/*
   ...
   Inputs      : parametersList = The parameters list to use while the AAI
                                  system is active.
...
/*
   ...
   Description : Determines if the two parameters reference valid memory
                 locations.
   Inputs      : processParameters = The parameters list used by the calling
                                     process.
...
int aaiValidateParameters(void ** processParameters, CARDINAL_16 * key) {
   ...
   return ((processParameters == NULL || key == NULL) ? EINVAL : EZERO);
...
/*
   ...
   Updates     : Nothing.
   Outputs     : processParameters     = The parameters list used by the
                                         calling process.
   Returns     : The value of the last error that occurred.  A value of zero
                 indicates that no errors occurred.
   Modified    : 08/18/92 DWP Initial coding.
...
/*
   ...
   Inputs      : Nothing.
   Updates     : processParameters = The parameters list used by this
                                     process.
      ...
      if (*(processParameters) == NULL) {
         ...
         */
         ...
         /*
            ...
            Toggle the blinking character between the hours and minutes
            fields of the displayed time.
         ...
         */
         ...
         if (parameters->secondsValue != currentTime->tm_sec || parameters->separatorCharacter == '\0') {
            ...
            /*
               ...
               Determine if it is before or after 12:00 PM, so that the 12
               hour format can be displayed on the screen.
...
/*
   ...
   Description : processes the Yes/No message used in various other
                 processes.
   Inputs      : Nothing.
   Updates     : processParameters = The parameters list used by this
                                     process.
...
/*
   ...
   Procedure   : aaiInitialize8255
   Description : Initializes the Intel 8255 located on ATI's milk analyzer
                 i/o card.
   Inputs      : processParameters = The parameters list used by this
                                     process.
   Updates     : Nothing.
   Outputs     : Nothing.
   Returns     : Nothing.
...
void aaiInitialize8255(AaiParametersList * processParameters) {
   ...
   /*
      ...
      Initializes the 8255 and the i/o card.

   */

   outportb(processParameters->defaultParameters.statusWord, 0x89);
   outportb(processParameters->defaultParameters.portA, 0x06);
   outportb(processParameters->defaultParameters.portB, 0x01);

}

/*

   Procedure   : aaiSelectAdcChannel
   Description : Selects one of the 8 channels located on ATI's milk analyzer
                 i/o card.
   Inputs      : processParameters = The parameters list used by this
                                     process.
                 channel           = The channel to select.  See notes.
   Updates     : Nothing.
   Outputs     : Nothing.
   Returns     : Nothing.
   Modified    : 12/17/94 DWP Initial coding.
   Notes       : 1)  Selects the adc channel to monitor.  Values are as
                     follows:
...
void selectAdcChannel(AaiParametersList * processParameters, unsigned int channel) {
   ...
   */
   ...
   outportb(processParameters->defaultParameters.portB, (channel << 3) | 0x01);

}

/*

   Procedure   : aaiStartAdcConversion
   Description : Tells the adc located on ATI's milk analyzer i/o card to
...
void aaiStartAdcConversion(AaiParametersList * processParameters) {
   ...
   */
   ...
   outport(processParameters->defaultParameters.adcUpper, 0x00);

}

/*

   Procedure   : aaiPollAdcUntilConversionIsFinished
   Description : Polls the adc in ATI's milk analzyer i/o card until the
...
void aaiPollAdcUntilConversionIsFinished(AaiParametersList * processParameters) {
   ...
   */
   ...
   while (adcBusyFlag) {
      ...
      if ((inportb(processParameters->defaultParameters.portC) & 0x10) == 0) {
...
/*
   ...
   Function    : aaiReadAdcValue
   Description : Gets the value that was most recently converted by the adc
                 from ATI's milk analyzer i/o card.
   Inputs      : processParameters = The parameters list used by this
                                     process.
   Updates     : Nothing.
   Outputs     : Nothing.
...
unsigned int aaiReadAdcValue(AaiParametersList * processParameters) {
   ...
   */
   ...
   return ((_rotr(inportb(processParameters->defaultParameters.adcLower), 4) + _rotl(inportb(processParameters->defaultParameters.adcUpper), 4)) & 0x7fff);

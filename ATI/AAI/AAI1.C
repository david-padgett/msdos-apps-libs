         ...
         if (!parameters->refreshFlag) {
            ...
            if (result == 0 && (result = PORT_Constructor(parameters->startingRow, parameters->startingColumn, parameters->height, parameters->width, PORT_SINGLE_LINE_BORDER, parameters->colors.primaryColor, parameters->colors.primaryColor, &(parameters->port))) == PORT_NOERR) {
               ...
               if ((result = MENU_Constructor(parameters->menuText, 1, 2, parameters->colors.primaryColor, parameters->colors.secondaryColor, parameters->colors.primaryHighlightColor, parameters->colors.secondaryHighlightColor, NULL, 0, 0, 0, 0, parameters->text, &(parameters->menu))) == MENU_NOERR) {
                  ...
                  if (strcmp(parameters->menuText, MENU_END_OF_TEXT) == 0) {
         ...
         if (result == EZERO) {
            ...
            switch (processNumber) {
               ...
               case AaiMeasureBatchProcess :
                  ...
                  measureBatchParameters->port = NULL;
                  measureBatchParameters->text = parametersList->text;
                  memcpy(&(measureBatchParameters->colors), &(parametersList->defaultParameters.mainMenuColors), sizeof(measureBatchParameters->colors));
                  measureBatchParameters->startingRow = 5;
                  measureBatchParameters->startingColumn = 2;
                  measureBatchParameters->height = 16;
                  measureBatchParameters->width = 78;

                  measureBatchParameters->laboratory.text = parametersList->text;
                  measureBatchParameters->laboratory.editLength = 50;
                  measureBatchParameters->laboratory.displayLength = 50;
                  measureBatchParameters->laboratory.type = STRING_TYPE;
                  memcpy(&(measureBatchParameters->laboratory.colors), &(parametersList->defaultParameters.mainMenuColors), sizeof(measureBatchParameters->laboratory.colors));
                  measureBatchParameters->laboratory.currentRow = 3;
                  measureBatchParameters->laboratory.currentColumn = 19;
                  measureBatchParameters->laboratory.markedFlag = FALSE;
                  measureBatchParameters->laboratory.refreshFlag = TRUE;

                  measureBatchParameters->operator.text = parametersList->text;
                  measureBatchParameters->operator.editLength = 50;
                  measureBatchParameters->operator.displayLength = 50;
                  measureBatchParameters->operator.type = STRING_TYPE;
                  memcpy(&(measureBatchParameters->operator.colors), &(parametersList->defaultParameters.mainMenuColors), sizeof(measureBatchParameters->operator.colors));
                  measureBatchParameters->operator.currentRow = 5;
                  measureBatchParameters->operator.currentColumn = 19;
                  measureBatchParameters->operator.markedFlag = FALSE;
                  measureBatchParameters->operator.refreshFlag = TRUE;

                  measureBatchParameters->dateAndTime.text = parametersList->text;
                  measureBatchParameters->dateAndTime.editLength = 50;
                  measureBatchParameters->dateAndTime.displayLength = 50;
                  measureBatchParameters->dateAndTime.type = STRING_TYPE;
                  memcpy(&(measureBatchParameters->dateAndTime.colors), &(parametersList->defaultParameters.mainMenuColors), sizeof(measureBatchParameters->dateAndTime.colors));
                  measureBatchParameters->dateAndTime.currentRow = 7;
                  measureBatchParameters->dateAndTime.currentColumn = 19;
                  measureBatchParameters->dateAndTime.markedFlag = FALSE;
                  measureBatchParameters->dateAndTime.refreshFlag = TRUE;

                  measureBatchParameters->program.text = parametersList->text;
                  measureBatchParameters->program.editLength = 50;
                  measureBatchParameters->program.displayLength = 50;
                  measureBatchParameters->program.type = STRING_TYPE;
                  memcpy(&(measureBatchParameters->program.colors), &(parametersList->defaultParameters.mainMenuColors), sizeof(measureBatchParameters->program.colors));
                  measureBatchParameters->program.currentRow = 9;
                  measureBatchParameters->program.currentColumn = 19;
                  measureBatchParameters->program.markedFlag = FALSE;
                  measureBatchParameters->program.refreshFlag = TRUE;

                  measureBatchParameters->typeOfSample.text = parametersList->text;
                  measureBatchParameters->typeOfSample.editLength = 50;
                  measureBatchParameters->typeOfSample.displayLength = 50;
                  measureBatchParameters->typeOfSample.type = STRING_TYPE;
                  memcpy(&(measureBatchParameters->typeOfSample.colors), &(parametersList->defaultParameters.mainMenuColors), sizeof(measureBatchParameters->typeOfSample.colors));
                  measureBatchParameters->typeOfSample.currentRow = 11;
                  measureBatchParameters->typeOfSample.currentColumn = 19;
                  measureBatchParameters->typeOfSample.markedFlag = FALSE;
                  measureBatchParameters->typeOfSample.refreshFlag = TRUE;

                  measureBatchParameters->batchCode.text = parametersList->text;
                  measureBatchParameters->batchCode.editLength = 50;
                  measureBatchParameters->batchCode.displayLength = 50;
                  measureBatchParameters->batchCode.type = STRING_TYPE;
                  memcpy(&(measureBatchParameters->batchCode.colors), &(parametersList->defaultParameters.mainMenuColors), sizeof(measureBatchParameters->batchCode.colors));
                  measureBatchParameters->batchCode.currentRow = 13;
                  measureBatchParameters->batchCode.currentColumn = 19;
                  measureBatchParameters->batchCode.markedFlag = FALSE;
                  measureBatchParameters->batchCode.refreshFlag = TRUE;
...
int aaiMain(AaiParametersList * parametersList) {
   ...
   */
   ...
   if ((result = aaiInitialize(parametersList)) == EZERO) {
      ...
      aaiInitialize8255(parametersList);
      ...
      */
      ...
      for (i = 1; i < argc; i++) {
         ...
         if (strchr("-/", argv[i][0]) != NULL) {
            ...
            if (stricmp("PORTA", &(argv[i][1])) == 0 && atoi(argv[i + 1]) > 0) {

               parametersList.defaultParameters.portA = atoi(argv[i + 1]);

            }

            if (stricmp("PORTB", &(argv[i][1])) == 0 && atoi(argv[i + 1]) > 0) {

               parametersList.defaultParameters.portB = atoi(argv[i + 1]);

            }

            if (stricmp("PORTC", &(argv[i][1])) == 0 && atoi(argv[i + 1]) > 0) {

               parametersList.defaultParameters.portC = atoi(argv[i + 1]);

            }

            if (stricmp("STATUSWORD", &(argv[i][1])) == 0 && atoi(argv[i + 1]) > 0) {

               parametersList.defaultParameters.statusWord = atoi(argv[i + 1]);

            }

            if (stricmp("ADCUPPER", &(argv[i][1])) == 0 && atoi(argv[i + 1]) > 0) {

               parametersList.defaultParameters.adcUpper = atoi(argv[i + 1]);

            }

            if (stricmp("ADCLOWER", &(argv[i][1])) == 0 && atoi(argv[i + 1]) > 0) {

               parametersList.defaultParameters.adcLower = atoi(argv[i + 1]);
